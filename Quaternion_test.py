...
# initialize some list to store data
X1 = []
X2 = []
MEANDIFF_Q = []
MINDIFF_Q = []
...
for N in range (1,30):
    ...
    X1.append(1.0 / N)
    X2.append(N)
    MEANDIFF_Q.append(meandiff_Q / 1000)
    MINDIFF_Q.append(mindiff_Q)
    MAXDIFF_Q.append(maxdiff_Q)
    MEANDIFF_DCM.append(meandiff_DCM / 1000)
...
ax.set_xlabel("Times")
ax.set_ylabel("Differences")
ax.set_title("Differences between DCM and Quaternion in 1000 steps, 30 rounds")
legend = ax.legend(loc = 2, bbox_to_anchor = (1.05, 1.0), borderaxespad = 0.)
legend.get_frame().set_facecolor('C0')
plt.show()
fig.set_size_inches(18.5, 10.5)
fig.savefig('test_DCM_Quaternion_1000.png', dpi = 100)
