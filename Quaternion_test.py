...
(X1, X2, MEANDIFF, MINDIFF, MAXDIFF) = f.Quaternion_rotation_precision(30, 10, np.pi / 2, np.pi / 2, np.pi / 2)
fig, ax = plt.subplots()
...
ax.set_title("Mean, Max, Min differences(Quaternion)")
legend = ax.legend(loc=2, bbox_to_anchor=(1.05,1.0),borderaxespad = 0.)
legend.get_frame().set_facecolor('C0')
plt.show()
fig.set_size_inches(18.5, 10.5)
fig.savefig('Quaternion_rotation_Precision.png', dpi = 100)

print("-------test on DCM rotation-------")
(X1, X2, MEANDIFF, MINDIFF, MAXDIFF) = f.DCM_rotation_precision(30, 10, np.pi / 2, np.pi / 2, np.pi / 2)
fig, ax = plt.subplots()
ax.semilogy(X1, MEANDIFF, 'k--', label='Mean', color = 'red')
ax.semilogy(X1, MINDIFF, 'k:', label='MIN', color = 'green')
ax.semilogy(X1, MAXDIFF, 'k', label='MAX', color = 'blue')
ax.set_xlabel("Times, 1/n")
ax.set_ylabel("Differences")
ax.set_title("Mean, Max, Min differences(DCM)")
legend = ax.legend(loc = 2, bbox_to_anchor = (1.05, 1.0), borderaxespad = 0.)
legend.get_frame().set_facecolor('C0')
plt.show()
fig.set_size_inches(18.5, 10.5)
fig.savefig('DCM_rotation_Precision.png', dpi = 100)

print("-------compare output differences in Quaternion and DCM-------")
(X1, X2, MEANDIFF_Q, MINDIFF_Q, MAXDIFF_Q, MEANDIFF_DCM, MINDIFF_Q, MAXDIFF_Q) = f.Quaternion_DCM_rotation_precision(30, 10, np.pi / 2, np.pi / 2, np.pi / 2)
fig, ax = plt.subplots()
...
plt.show()
fig.set_size_inches(18.5, 10.5)
fig.savefig('DCM_Quaternion_Precision.png', dpi = 100)
...
(X1, X2, DIAGONAL_CHECK_Q, OFF_DIAGONAL_CHECK_Q, ORTHONORMALITY_COL_Q, ORTHONORMALITY_ROW_Q, DIAGONAL_CHECK_DCM, OFF_DIAGONAL_CHECK_DCM, ORTHONORMALITY_COL_DCM, ORTHONORMALITY_ROW_DCM) = f.DCM_Quaternion_rotator_check(30, 10, np.pi / 2, np.pi / 2, np.pi / 2)
#x1 = TemporaryFile()
#np.save(x1, X1)
#x1.seek(0)
#np.load(x1)
...
ax.set_xlabel("Times")
ax.set_ylabel("Differences")
ax.set_title("DCM and Quaternion rotator check, Manhattan Distance")
legend = ax.legend(loc = 2, bbox_to_anchor=(1.05,1.0),borderaxespad = 0.)
legend.get_frame().set_facecolor('C0')
plt.show()
fig.set_size_inches(18.5, 10.5)
fig.savefig('DCM_Quaternion_MD.png', dpi = 100)